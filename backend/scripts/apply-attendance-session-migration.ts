/**
 * Applies attendance session columns to office_locations (additive only).
 */
...
const migrationPath = join(__dirname, "../supabase/apply_attendance_session_migration.sql");
const migrationSql = fs.readFileSync(migrationPath, "utf8");

async function main() {
  console.log("Applying attendance session migration (additive)...");
  await sql.unsafe(migrationSql);
  console.log("Attendance session migration applied successfully.");
  await sql.end();
}
