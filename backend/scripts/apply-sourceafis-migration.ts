/**
 * Applies `backend/supabase/apply_sourceafis_migration.sql` to Aiven Postgres only.
...
const migrationPath = join(__dirname, "../supabase/apply_sourceafis_migration.sql");
const migrationSql = fs.readFileSync(migrationPath, "utf8");

async function main() {
  console.log("Applying SourceAFIS fingerprint migration (additive, Aiven-only)...");
  await sql.unsafe(migrationSql);
  console.log("SourceAFIS migration applied successfully.");
  await sql.end();
}
